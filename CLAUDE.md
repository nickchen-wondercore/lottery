...
| `renderer.js` → `Renderer` | Canvas 2D 自訂繪製：分層渲染（出口管 → 斜坡 → 容器填充 → 球體 → 容器邊框），安全繪製包裝 |
...
- **亂流系統**：關閉重力，施加旋轉力 + 噪音擾動 + 隨機爆發力 + 居中力 + 速度限制器，強度由「氣流」滑桿控制（1-10）
- **繪製分離**：`Renderer` 不操作 DOM，僅透過 Canvas API 繪製；`Physics` 不知道 UI，透過 callback 回傳結果

### 資料流

`app.js` 呼叫 `Physics` API（`createBalls`、`startTurbulence`、`ejectOneBall` 等），每幀呼叫 `Renderer.draw()` 並傳入 `Physics` 狀態。`names.json` 為純 JSON 陣列，fetch 載入後用於球體生成。
